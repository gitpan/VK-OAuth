# This Makefile is for the VK::OAuth extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/VK/OAuth.pm]
#     AUTHOR => [q[Alexander Babenko <foxcool@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[VK::OAuth]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], LWP::UserAgent=>q[0], JSON=>q[0], Carp=>q[0] }
#     VERSION_FROM => q[lib/VK/OAuth.pm]
#     clean => { FILES=>q[VK-OAuth-*] }
...
OBJ_EXT = .o
OSNAME = linux
OSVERS = 2.6.32-279.2.1.el6.x86_64
RANLIB = :
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/VK/OAuth.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/VK
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/VK
...
TO_INST_PM = lib/VK/OAuth.pm

PM_TO_BLIB = lib/VK/OAuth.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/VK/OAuth.pm $(INST_MAN3DIR)/VK::OAuth.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               VK-OAuth' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.02' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           OAuth authorization on your site with VK API' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Alexander Babenko <foxcool@cpan.org>' >> META_new.yml
...
disttest : distdir
	cd $(DISTVNAME) && $(ABSPERLRUN) Makefile.PL "-h"
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>OAuth authorization on your site with VK API</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Alexander Babenko &lt;foxcool@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/VK/OAuth.pm blib/lib/VK/OAuth.pm 
